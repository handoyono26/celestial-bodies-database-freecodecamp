...
echo "$($PSQL "SELECT name FROM teams WHERE team_id = (SELECT winner_id FROM games WHERE winner_id = team_id AND year = 2014 AND round = 'Eighth-Final') OR team_id = (SELECT opponent_id FROM games WHERE opponent_id = team_id AND year = 2014 AND round = 'Eighth-Final') ORDER BY name")"
